...
class PurchaseController extends Controller
{
    ...
    public function storePurchaseEvidence(Request $request, $id)
    {
        ...
        // ==========================================================
        // 2. GENERATOR KODE (SINKRONISASI MODEL CATEGORY)
        // ==========================================================
        
        // Ambil data kategori berdasarkan ID di pembelian
        $category = Category::find($purchase->category_id);

        // Default kalau kategori tidak ditemukan atau kosong
        // Saya ganti jadi XXX biar beda dari 'TOOL' atau 'GEN'
        $prefix = 'XXX'; 
        
        if ($category) {
            // Cek nama kategori (prioritas: category_name -> name)
            // Kita paksa ambil string-nya
            $rawName = $category->category_name ?? $category->name;
            
            if (!empty($rawName)) {
                // Ambil 3 huruf depan, uppercase (Contoh: "Laptop" -> "LAP")
                $prefix = strtoupper(substr($rawName, 0, 3)); 
            }
        }

        // Cari nomor urut terakhir di tabel Tools berdasarkan prefix yang didapat
